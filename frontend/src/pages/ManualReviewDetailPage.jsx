...
export default function ManualReviewDetailPage() {
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col bg-slate-950 text-slate-100 font-sans items-center justify-center p-12">
        <p className="text-slate-400">Loading inspection details...</p>
      </div>
    );
  }
  ...
  if (!review) {
    return (
      <div className="flex-1 flex flex-col bg-slate-950 text-slate-100 font-sans p-6 space-y-4">
    ...
    <div className="flex-1 flex flex-col min-w-0 bg-slate-950 text-slate-100 font-sans">
      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto">
        <button
          ...
          className="px-3 py-1.5 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white rounded-xl text-xs flex items-center gap-2 font-mono"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Manual Review Queue
        </button>

        {/* 1. Header Status Bar */}
        <div className="bg-slate-900/80 rounded-xl border border-slate-800 p-5 backdrop-blur-md flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Raw OCR: <span className="font-mono text-amber-400">{review.raw_ocr_text || review.recognized_plate}</span></p>
          </div>
          ...
          <div className="flex items-center gap-3 font-mono text-xs">
            ...
            <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
              <span className="text-slate-500 text-[10px] block">Review Status</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-slate-900/80 rounded-xl border border-slate-800 p-5 backdrop-blur-md space-y-3">
            ...
            {review.vehicle_image_path ? (
              <img
                ...
                alt="Vehicle Crop"
                className="w-full rounded-xl border border-slate-800 bg-slate-950 max-h-64 object-contain"
                onError={(e) => { e.target.style.display = 'none'; }}
              />
            ) : (
              <div className="p-8 text-center text-slate-500 bg-slate-950 rounded-xl border border-slate-800 text-xs">Vehicle Crop Available</div>
            )}
          </div>
          ...
          <div className="bg-slate-900/80 rounded-xl border border-slate-800 p-5 backdrop-blur-md space-y-3">
            ...
            {review.plate_image_path ? (
              ...
              <div className="p-8 text-center text-slate-500 bg-slate-950 rounded-xl border border-slate-800 text-xs">Plate Crop Available</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 rounded-xl border border-slate-800 p-5 backdrop-blur-md space-y-4">
          ...
          <div className="bg-slate-950 rounded-xl border border-slate-800 p-4 space-y-3 font-mono text-xs">
            {!review.corrections_history || review.corrections_history.length === 0 ? (
              <p className="text-slate-500 text-center py-2 font-sans">No manual corrections applied yet.</p>
            ) : (
              review.corrections_history.map((c) => (
                <div key={c.id} className="flex items-center justify-between border-b border-slate-800/60 pb-2 text-[11px]">
                  <div>
                    <span className="text-slate-400">{c.old_plate}</span> → <span className="text-cyan-400 font-bold text-sm">{c.new_plate}</span>
                    <p className="text-slate-500 font-sans text-[10px] pt-0.5">{c.correction_reason}</p>
                  </div>
        ...
        <div className="bg-slate-900/80 rounded-xl border border-slate-800 p-5 backdrop-blur-md flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Database className="w-5 h-5 text-emerald-400" />
            <div>
              <h4 className="text-xs font-bold text-white">AI Retraining Feedback Dataset Collector</h4>
              <p className="text-[11px] text-slate-400">Sample exported to <span className="font-mono text-cyan-400">backend/app/ai/feedback_dataset/</span> for OCR model retraining</p>
            </div>
          </div>
