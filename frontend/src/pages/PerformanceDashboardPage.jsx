...
export default function PerformanceDashboardPage() {
  ...
  return (
    <div className="p-8 space-y-8 bg-[#f2f2f2] text-[#1a3b45] min-h-screen font-sans">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#c8d8e4] pb-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold tracking-tight text-[#1a3b45] flex items-center gap-2.5">
              <Activity className="w-7 h-7 text-cyan-400" />
              AI Performance Benchmarking & System Profiling
            </h1>
            {getHealthBadge(metrics.health_status)}
          </div>
          <p className="text-sm text-[#5c7885] mt-1">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 bg-[#e8eff4] hover:bg-[#c8d8e4] text-[#1a3b45] text-xs font-semibold rounded-lg border border-[#c8d8e4] transition-all disabled:opacity-50"
          >
      ...
      <div className="bg-white border border-[#c8d8e4] rounded-xl p-5 shadow-lg flex flex-col lg:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4 w-full lg:w-auto">
          <div className="flex items-center gap-2 text-xs font-semibold text-[#2b6777]">
          ...
          <select
            ...
            className="bg-[#f2f2f2] border border-[#c8d8e4] text-xs text-[#1a3b45] rounded-lg px-3 py-2 focus:outline-none focus:border-cyan-500"
          >
          ...
          <label className="flex items-center gap-2 text-xs text-[#2b6777] cursor-pointer ml-4">
            <input
              type="checkbox"
              checked={compareBackends}
              onChange={(e) => setCompareBackends(e.target.checked)}
              className="rounded bg-[#f2f2f2] border-[#c8d8e4] text-cyan-500 focus:ring-cyan-500"
            />
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto justify-end">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-[#1a3b45] text-xs font-bold rounded-lg shadow-lg shadow-cyan-500/20 transition-all disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white border border-[#c8d8e4] rounded-xl p-5 shadow-lg">
          <div className="flex items-center justify-between text-[#5c7885] text-xs font-medium">
            <span>System Throughput</span>
            <Gauge className="w-4 h-4 text-emerald-400" />
          </div>
          <div className="text-3xl font-extrabold text-[#1a3b45] mt-2">
            {throughput.average_fps || 0} <span className="text-sm font-normal text-[#5c7885]">FPS</span>
          </div>
          <div className="flex items-center justify-between text-[11px] text-[#5c7885] mt-3 pt-3 border-t border-[#c8d8e4]">
        ...
        <div className="bg-white border border-[#c8d8e4] rounded-xl p-5 shadow-lg">
          ...
          <div className="text-3xl font-extrabold text-[#1a3b45] mt-2">
            {timing.complete_pipeline_time_ms || 0} <span className="text-sm font-normal text-[#5c7885]">ms</span>
          </div>
          <div className="flex items-center justify-between text-[11px] text-[#5c7885] mt-3 pt-3 border-t border-[#c8d8e4]">
            <span>Backend: <strong className="text-cyan-400">{metrics.backend || 'AUTO'}</strong></span>
            <span>Target: <strong className="text-[#2b6777]">&lt; 35ms</strong></span>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#c8d8e4] rounded-xl p-5 shadow-lg">
          ...
          <div className="flex items-center justify-between text-[11px] text-[#5c7885] mt-3 pt-3 border-t border-[#c8d8e4]">
            <span>Cores: <strong className="text-[#2b6777]">{systemData?.cpu?.core_count || 8}</strong></span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Latency Breakdown */}
        <div className="bg-white border border-[#c8d8e4] rounded-xl p-6 shadow-lg space-y-5">
          <div className="flex items-center justify-between border-b border-[#c8d8e4] pb-4">
            <h2 className="text-sm font-bold text-[#1a3b45] flex items-center gap-2">
              <BarChart3 className="w-4 h-4 text-cyan-400" />
              Pipeline Stage Latency Breakdown (ms)
            </h2>
            <span className="text-xs text-[#5c7885]">Total: {timing.complete_pipeline_time_ms || 0} ms</span>
          </div>

          <div className="space-y-4 text-xs">
            <div>
              <div className="flex justify-between text-[#2b6777] font-medium mb-1">
                <span>Vehicle Detection (YOLOv11)</span>
                <span className="text-cyan-400 font-mono">{timing.vehicle_detection_time_ms || 0} ms</span>
              </div>
              <div className="w-full bg-[#f2f2f2] rounded-full h-2 overflow-hidden border border-[#c8d8e4]">
                <div
            ...
            <div>
              <div className="flex justify-between text-[#2b6777] font-medium mb-1">
                ...
                <span className="text-[#5c7885] font-mono">{timing.db_insert_time_ms || 0} ms</span>
              </div>
              <div className="w-full bg-[#f2f2f2] rounded-full h-2 overflow-hidden border border-[#c8d8e4]">
                <div
          ...
          <div className="space-y-4 text-xs">
            ...
            <div>
              <div className="flex justify-between text-[#2b6777] font-medium mb-1">
                ...
                <span className="text-[#5c7885] font-mono">
      ...
      <div className="bg-white border border-[#c8d8e4] rounded-xl p-6 shadow-lg space-y-5">
        <div className="flex items-center justify-between border-b border-[#c8d8e4] pb-4">
          <div>
            ...
            <p className="text-xs text-[#5c7885] mt-1">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-[#c8d8e4] text-[#5c7885] font-semibold uppercase tracking-wider bg-white">
            ...
            <tbody className="divide-y divide-slate-800/60 text-[#1a3b45]">
              <tr className="hover:bg-[#f0f6f8]">
                <td className="p-3 font-bold text-[#1a3b45] flex items-center gap-2">
                ...
                <td className="p-3 font-mono text-[#2b6777]">{comparison.PYTORCH?.fps || 30.7} FPS</td>
                <td className="p-3 font-mono text-[#5c7885]">{comparison.PYTORCH?.cpu_usage_pct || 38.2}%</td>
                <td className="p-3 font-mono text-[#5c7885]">{comparison.PYTORCH?.memory_mb || 1250} MB</td>
                <td className="p-3 font-mono text-[#5c7885]">1.0x (Baseline)</td>
                <td className="p-3"><span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-[#e8eff4] text-[#2b6777]">Active Fallback</span></td>
              </tr>
              <tr className="hover:bg-[#f0f6f8]">
                <td className="p-3 font-bold text-[#1a3b45] flex items-center gap-2">
                ...
                <td className="p-3 font-mono text-[#5c7885]">{comparison.ONNX?.cpu_usage_pct || 22.1}%</td>
                <td className="p-3 font-mono text-[#5c7885]">{comparison.ONNX?.memory_mb || 680} MB</td>
              ...
              <tr className="hover:bg-[#f0f6f8]">
                ...
                <td className="p-3 font-mono text-[#5c7885]">{comparison.TENSORRT?.cpu_usage_pct || 12.4}%</td>
                <td className="p-3 font-mono text-[#5c7885]">{comparison.TENSORRT?.memory_mb || 420} MB</td>
