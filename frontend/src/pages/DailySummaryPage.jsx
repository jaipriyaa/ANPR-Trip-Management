      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto">
        <div className="flex items-center justify-between bg-slate-900/80 rounded-xl p-4 border border-slate-800 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-white">Daily Operational Aggregates</h2>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono">
                {loading ? (
                  <tr>
                    <td colSpan="9" className="p-8 text-center text-slate-500 font-sans">Loading daily summaries...</td>
                  </tr>
                ) : summaries.length === 0 ? (
                  <tr>
                    <td colSpan="9" className="p-8 text-center text-slate-500 font-sans">No daily summary records found.</td>
                  </tr>
                ) : (
                  summaries.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="p-4 text-slate-300">{s.avg_stay_duration_mins} mins</td>
