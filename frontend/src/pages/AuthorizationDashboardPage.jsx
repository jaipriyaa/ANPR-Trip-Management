      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto">
        ...
        <div className="flex items-center justify-between bg-slate-900/80 rounded-xl p-4 border border-slate-800 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-white">Central Security Decision Engine</h2>
              <p className="text-xs text-slate-400">Live 5s auto-refresh active</p>
            </div>
          </div>

          <button
            onClick={fetchDashboard}
            className="px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-300 hover:text-white text-xs flex items-center gap-2"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <div className="bg-slate-900/60 rounded-xl p-3 border border-emerald-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Authorized Today</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-rose-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Denied Today</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-purple-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Manual Approvals</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-amber-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Unknown Vehicles</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-rose-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Watchlist Hits</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-cyan-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Whitelist Count</p>
        ...
        <div className="bg-slate-900/80 rounded-xl border border-slate-800 p-5 space-y-4 backdrop-blur-md">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Pending unregistered & manual verification vehicle access requests</p>
            </div>
          </div>

          <div className="space-y-3 font-mono text-xs">
            {!data?.pending_manual_queue || data.pending_manual_queue.length === 0 ? (
              <p className="text-slate-500 text-center py-6 font-sans">No pending manual review requests at gate.</p>
            ) : (
              data.pending_manual_queue.map((item) => (
                <div key={item.id} className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-center justify-between flex-wrap gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-slate-300 font-sans text-xs">{item.reason}</p>
                    <p className="text-[10px] text-slate-500">Timestamp: {new Date(item.time).toLocaleString()}</p>
                  </div>
