  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-[#f2f2f2] text-[#1a3b45] font-sans">
      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto">
        {/* Tab Navigation */}
        <div className="flex items-center gap-2 border-b border-[#c8d8e4] pb-3">
          <button
            onClick={() => setActiveTab('camera')}
            className={`px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2 transition-all ${
              activeTab === 'camera'
                ? 'bg-cyan-500 text-slate-950 shadow-lg shadow-cyan-500/20'
                : 'bg-white text-[#5c7885] border border-[#c8d8e4] hover:text-[#1a3b45]'
            }`}
          >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {cameraHealth.map((cam) => (
                <div key={cam.camera_id} className="bg-white rounded-xl border border-[#c8d8e4] p-5 space-y-3 backdrop-blur-md">
                  <div className="flex items-center justify-between border-b border-[#c8d8e4] pb-2">
                    <div>
                      <h4 className="font-bold text-sm text-[#1a3b45]">{cam.camera_name}</h4>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                    <div className="bg-[#f2f2f2] p-2 rounded-lg border border-[#c8d8e4]">
                      <span className="text-[#5c7885] text-[10px] block">Frame Rate</span>
                      <span className="font-bold text-cyan-400">{cam.fps} FPS</span>
                    </div>
                    <div className="bg-[#f2f2f2] p-2 rounded-lg border border-[#c8d8e4]">
                      <span className="text-[#5c7885] text-[10px] block">Stream Latency</span>
                      <span className="font-bold text-emerald-400">{cam.latency_ms} ms</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between text-xs text-[#5c7885] pt-1">
                    <span>Resolution: {cam.resolution}</span>
                    <span className="text-[#5c7885] font-mono">Frame: {cam.last_frame_time}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'model' && modelHealth && (
          <div className="bg-white rounded-xl border border-[#c8d8e4] p-6 space-y-6 backdrop-blur-md">
            <div className="flex items-center justify-between border-b border-[#c8d8e4] pb-4">
              <div>
                ...
                <p className="text-xs text-[#5c7885] pt-0.5">Model Version: <span className="font-mono text-cyan-400">{modelHealth.model_version}</span></p>
              </div>
        ...
        {activeTab === 'settings' && (
          <form onSubmit={handleSaveSettings} className="bg-white rounded-xl border border-[#c8d8e4] p-6 space-y-6 backdrop-blur-md max-w-2xl">
            <h3 className="text-base font-bold text-[#1a3b45] flex items-center gap-2 border-b border-[#c8d8e4] pb-3">
              <Settings className="w-5 h-5 text-cyan-400" /> Platform Configuration Thresholds
            </h3>

            <div className="space-y-4 text-xs">
              <div>
                <label className="block text-[#2b6777] font-semibold mb-1">Recognition Confidence Threshold (0.50 - 1.00)</label>
                <input
                  type="text"
                  ...
                  className="w-full bg-[#f2f2f2] border border-[#c8d8e4] rounded-xl p-2.5 text-[#1a3b45] font-mono"
                />
              </div>

              <div>
                <label className="block text-[#2b6777] font-semibold mb-1">Duplicate Suppression Window (Seconds)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[#2b6777] font-semibold mb-1">Maximum File Upload Limit (MB)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[#2b6777] font-semibold mb-1">Data Retention Period (Days)</label>
                <input
                  type="text"
            ...
            <div className="pt-4 border-t border-[#c8d8e4]">
              <button
                type="submit"
        ...
        {activeTab === 'alerts' && (
          <div className="bg-white rounded-xl border border-[#c8d8e4] p-5 space-y-4 backdrop-blur-md">
            ...
            <div className="space-y-3 font-mono text-xs">
              <div className="bg-[#f2f2f2] p-4 rounded-xl border border-rose-500/30 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="px-2 py-0.5 bg-rose-500/10 text-rose-400 font-bold rounded">UNAUTHORIZED ENTRY ATTEMPT</span>
                  <p className="text-[#1a3b45] font-sans text-xs">Unregistered truck plate <span className="font-mono text-cyan-400">MH12AB9999</span> attempted entry at Gate North 01</p>
                  <span className="text-[10px] text-[#5c7885]">10 minutes ago</span>
                </div>
              ...
              <div className="bg-[#f2f2f2] p-4 rounded-xl border border-amber-500/30 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="px-2 py-0.5 bg-amber-500/10 text-amber-400 font-bold rounded">LOW CONFIDENCE WARNING</span>
                  <p className="text-[#1a3b45] font-sans text-xs">OCR confidence 64% on vehicle at East Gate 03</p>
                  <span className="text-[10px] text-[#5c7885]">25 minutes ago</span>
                </div>
                <button className="px-3 py-1.5 bg-[#e8eff4] text-[#2b6777] border border-[#c8d8e4] rounded-lg text-xs font-bold font-sans hover:bg-[#c8d8e4]">
                  Acknowledge
                </button>
              </div>
            </div>
