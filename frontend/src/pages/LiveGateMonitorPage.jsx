      ...
      <main className="flex-1 p-6 space-y-6 overflow-y-auto">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900/80 rounded-xl p-4 border border-slate-800 backdrop-blur-md">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <span className="text-xs text-slate-400 font-mono hidden sm:inline">Last Sync: {data?.timestamp || 'Just now'}</span>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-blue-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Entered Today</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-slate-700 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Exited Today</p>
            <p className="text-2xl font-extrabold text-slate-300 font-mono mt-1">{summary.vehicles_exited_today}</p>
            <p className="text-[10px] text-slate-400 font-mono mt-0.5">Outbound</p>
          </div>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-red-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Unauthorized</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-amber-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Live Alerts</p>
          ...
          <div className="bg-slate-900/60 rounded-xl p-3 border border-purple-500/30 backdrop-blur-md">
            <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Avg Stay Time</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400 font-mono">{cameras.length} Cameras Online</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {cameras.length === 0 ? (
                <div className="col-span-2 bg-slate-900/60 border border-slate-800 rounded-xl p-8 text-center text-slate-500">
                ...
                cameras.map((cam) => {
                  ...
                  return (
                    <div key={cam.camera_id} className="bg-slate-900/80 border border-slate-800 rounded-xl overflow-hidden shadow-xl backdrop-blur-md flex flex-col">
                      {/* Camera Stream Header */}
                      <div className="p-3 bg-slate-950 border-b border-slate-800/80 flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                          <span className="font-bold text-white">{cam.camera_name}</span>
                      ...
                      <div className="relative aspect-video bg-slate-950 flex items-center justify-center overflow-hidden border-b border-slate-800">
                        {curVeh?.cropped_vehicle_path ? (
                          ...
                          <div className="w-full h-full bg-gradient-to-tr from-slate-950 via-slate-900 to-slate-950 flex flex-col items-center justify-center p-4">
                            ...
                            <span className="text-xs font-mono text-slate-400">RTSP Stream Live Feed ({cam.resolution})</span>
                        ...
                        {ov && (
                          <div className="absolute inset-0 pointer-events-none p-4 flex flex-col justify-between">
                            <div className="self-start bg-slate-950/90 backdrop-blur-md border border-cyan-500/60 rounded px-2.5 py-1 text-[11px] font-mono text-cyan-300 shadow-lg">
                              <span className="text-emerald-400 font-bold">DETECTION:</span> {ov.recognized_plate} ({(ov.confidence * 100).toFixed(1)}%)
                            </div>

                            <div className="self-end bg-slate-950/90 backdrop-blur-md border border-emerald-500/60 rounded px-2 py-0.5 text-[10px] font-mono text-emerald-400">
                      ...
                      {/* Stream Footer Info */}
                      <div className="p-2.5 bg-slate-950/60 flex items-center justify-between text-[11px] text-slate-400 font-mono">
                        <span className="flex items-center gap-1"><MapPin className="w-3 h-3 text-slate-500" /> {cam.gate_name}</span>
          ...
          <div className="space-y-4">
            ...
            {curVeh ? (
              <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-5 space-y-4 backdrop-blur-md shadow-2xl">
                {/* Crops Row */}
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-400 uppercase font-semibold">Vehicle Crop</span>
                    <div className="h-24 rounded-lg overflow-hidden border border-slate-800 bg-slate-950 flex items-center justify-center">
                      {curVeh.cropped_vehicle_path ? (
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-400 uppercase font-semibold">Plate Crop</span>
                    <div className="h-24 rounded-lg overflow-hidden border border-slate-800 bg-slate-950 flex items-center justify-center">
                ...
                <div className="bg-slate-950 rounded-xl p-3 border border-cyan-500/30 flex items-center justify-between">
                  <div>
                    <p className="text-[10px] text-slate-400 uppercase font-semibold">Recognized License Plate</p>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
                    ...
                    <span className="font-semibold text-white">{curVeh.vehicle_type}</span>
                  </div>

                  <div className="bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
                    <span className="text-slate-400 text-[10px] block">Entry Gate</span>
                    <span className="font-semibold text-blue-400">{curVeh.entry_gate_code}</span>
                  </div>

                  <div className="bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
                    <span className="text-slate-400 text-[10px] block">Entry Timestamp</span>
                  ...
                  <div className="bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
                    <span className="text-slate-400 text-[10px] block">Stay Duration</span>
                  ...
                  <div className="bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80 col-span-2">
                    <span className="text-slate-400 text-[10px] block">Driver & Transporter</span>
                    <span className="font-semibold text-white">{curVeh.driver_name}</span>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl overflow-hidden backdrop-blur-md">
          {/* Tab Header */}
          <div className="flex items-center gap-2 border-b border-slate-800 bg-slate-950 px-4 pt-3 overflow-x-auto">
            <button
          ...
          <div className="p-4">
                ...
                <table className="w-full text-left text-xs text-slate-300">
                  ...
                  <tbody className="divide-y divide-slate-800/60 font-mono">
                    {insideVehicles.length === 0 ? (
                      <tr>
                        <td colSpan="8" className="p-6 text-center text-slate-500 font-sans">No vehicles currently inside facility.</td>
                      </tr>
                    ) : (
                      insideVehicles.map((m) => (
                        <tr key={m.id} className="hover:bg-slate-800/40">
                          <td className="p-3 font-bold text-cyan-400">{m.recognized_plate}</td>
                          <td className="p-3 text-white font-sans">{m.vehicle_type || 'SUV'}</td>
                          <td className="p-3 text-blue-400">{m.entry_gate_code || 'GATE-NORTH-01'}</td>
                          <td className="p-3 text-slate-400">{new Date(m.entry_time).toLocaleTimeString()}</td>
                          <td className="p-3 text-emerald-400 font-bold">{m.stay_duration_formatted || 'In Progress'}</td>
                          <td className="p-3 text-slate-300 font-sans">{m.destination || 'Main Assembly Bay'}</td>
                          <td className="p-3 text-slate-300 font-sans">{m.driver_name || 'Driver'}</td>
            ...
            {activeTab === 'TRIPS' && (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-slate-300">
                  ...
                  <tbody className="divide-y divide-slate-800/60 font-mono">
                    {activeTrips.map((t) => (
                      <tr key={t.trip_id} className="hover:bg-slate-800/40">
                        <td className="p-3 font-bold text-purple-400">{t.trip_id}</td>
                        <td className="p-3 font-bold text-cyan-400">{t.scheduled_vehicle}</td>
                        <td className="p-3 text-white font-sans">{t.vehicle_type}</td>
                        <td className="p-3 text-blue-400">{t.expected_gate}</td>
                        <td className="p-3 text-slate-400">{t.expected_entry_time}</td>
                        <td className="p-3 text-slate-400">{t.expected_exit_time}</td>
                        <td className="p-3 text-slate-300 font-sans">{t.purpose}</td>
                        <td className="p-3 text-right">
            ...
            {activeTab === 'TIMELINE' && (
              <div className="space-y-3 font-mono">
                {timeline.length === 0 ? (
                  <p className="text-center text-slate-500 p-6 font-sans">No live timeline events logged.</p>
                ) : (
                  timeline.map((evt) => (
                    <div key={evt.id} className="flex items-center justify-between bg-slate-950 p-3 rounded-xl border border-slate-800 text-xs">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-slate-400 font-sans">({evt.vehicle_type})</span>
                      </div>
                      <span className="text-slate-400 text-[11px]">{evt.gate_code}</span>
                    </div>
                  ))
                )}
              </div>
            ...
            {activeTab === 'ALERTS' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {alerts.length === 0 ? (
                  <div className="col-span-2 text-center text-slate-500 p-6">No active security alerts.</div>
                ) : (
                  alerts.map((alt) => (
